...
const { languages, defaultLocale } = require('../src/i18n/locales');
...
for (const lang of languages) {
  ...
  const translationFileName = `src/i18n/locales/${locale}.json`;
  try {
...
memoryTask.then(files => {
  ...
  extractTask.then(() => {
    ...
    mkdirp('src/i18n/locales', (err) => {
      ...
      for (const lang of languages) {
        const locale = lang.value;
        translationFileName = `src/i18n/locales/${locale}.json`;
        localeTaskDone = task(
